...
async function cargarPedidosPendientesHoy() {
    try {
        const res = await fetch('http://localhost:8080/api/v1/pedidos/activos?area=Cocina', {
...
function conectarWebSocket() {
    ...
    stompClient.connect({}, function (frame) {
        ...
        stompClient.subscribe('/topic/kds/cocina', function (mensaje) {
            const nuevoPedido = JSON.parse(mensaje.body);
            inyectarTicketEnUI(nuevoPedido, 'PENDIENTE');
        });
...
// 4. ACCIÓN HACIA EL BACKEND (Actualizado para recargar la página y evitar errores visuales)
async function avanzarEstado(idPedido, nuevoEstado) {
    try {
        ...
        if (res.ok) {
            // Recargamos la vista para traer los datos frescos
...
// --- ZONA GLOBAL (CRONÓMETRO) ---

// Objeto global para guardar a qué hora entró cada ticket
const tiemposTickets = {}; 

// Nota: Ya no hay código suelto de "tiemposTickets[pedido..." aquí afuera

setInterval(() => {
    const ahora = new Date();
    
    // Recorremos todos los tickets guardados
    for (const [id, horaCreacion] of Object.entries(tiemposTickets)) {
        ...
        if (spanTimer) {
            ...
            // Si pasan más de 10 minutos, lo pintamos más rojo
            if (minutos >= 10) {
